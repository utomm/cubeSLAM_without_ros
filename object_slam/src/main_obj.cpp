	...
	{      // row by row
		...
		if (pix_depth > close_depth_thre && pix_depth < far_depth_thre)
		{
			...
			Eigen::VectorXf global_pt = homo_to_real_coord_vec<float>(
				transToWorld * Eigen::Vector4f(pt.x, pt.y, pt.z, 1));  // change to global position
			pt.x = global_pt(0);
			pt.y = global_pt(1);
			pt.z = global_pt(2);
...
visualization_msgs::MarkerArray cuboids_to_marker(object_landmark* obj_landmark, Vector3d rgbcolor)
{
	...
	for (int ii = 0; ii < 2;
		 ii++) // each cuboid needs two markers!!! one for all edges, one for front facing edge, could with different color.
	{
		marker.id++;
	...
	std::vector<object_landmark*> all_frame_rawcubes, Eigen::MatrixXd& truth_frame_poses)
{
	...
	ros::Publisher
		pub_final_opti_cube = n.advertise<visualization_msgs::MarkerArray>("/cubes_opti", 10); //final unique cube pose
	ros::Publisher pub_history_opti_cube = n.advertise<visualization_msgs::MarkerArray>("/cubes_opti_hist",
...
// 			  all_pred_pose_odoms[frame_number].pose.pose.position.y <<std::endl;
			...
			if (show_truth_cloud && (truth_frame_poses.rows() > 0))
				...
				{
					std::string raw_depth_img_name =
						base_folder + "depth_imgs/" + std::string(frame_index_c) + "_depth_raw.png";
					...
					Eigen::Matrix4f truth_pose_matrix =
						g2o::SE3Quat(truth_frame_poses.row(frame_number).segment<7>(1)).to_homogeneous_matrix()
							.cast<float>();

					depth_to_cloud(raw_rgb_img,
						raw_depth_img,
						truth_pose_matrix,
						point_cloud,
	...
	Viewer& viewer)
{
	...
	std::vector<object_landmark*>
		cube_pose_opti_history(total_frame_number, nullptr);  //landmark pose after each frame's optimization
	std::vector<object_landmark*> cube_pose_raw_detected_history
		(total_frame_number, nullptr); //raw detected cuboid frame each frame. before optimization
		...
		// for (int frame_index = 0; frame_index < 5; frame_index++)
	{
		...
		if (online_detect_mode)
		{
			...
			if (!read_all_number_txt(base_folder + "/filter_2d_obj_txts/" + frame_index_c + "_yolo2_0.15.txt",
				raw_2d_objs))
				return;
			raw_2d_objs.leftCols<2>().array() -= 1;   // change matlab coordinate to c++, minus 1

			Matrix4d transToWolrd;
			detect_cuboid_obj.whether_sample_cam_roll_pitch = (frame_index
				!= 0); // first frame doesn't need to sample cam pose. could also sample. doesn't matter much
			...
			std::vector<ObjectSet>
				frames_cuboids; // each 2d bbox generates an ObjectSet, which is vector of sorted proposals
			...
			if (has_detected_cuboid)  // prepare object measurement
			{
				...
				cube_pose << detected_cube->pos(0), detected_cube->pos(1), detected_cube->pos(2),
					0, 0, detected_cube->rotY, detected_cube->scale(0), detected_cube->scale(1), detected_cube
					->scale(2);  // xyz roll pitch yaw scale
				cube_ground_value.fromMinimalVector(cube_pose);
				cube_local_meas =
					cube_ground_value.transform_to(curr_cam_pose_Twc); // measurement is in local camera frame

				if (detect_cuboid_obj
					.whether_sample_cam_roll_pitch)  //if camera roll/pitch is sampled, transform to the correct camera frame.
				{
					...
					Matrix3d rotation_new =
						euler_zyx_to_rot<double>(new_camera_eulers(0), new_camera_eulers(1), new_camera_eulers(2));
		...
		else
		{
			...
			if (has_detected_cuboid)  // prepare object measurement   not all frame has observation!!
			{
				...
				Vector9d cube_pose;
				cube_pose << measure_data(1), measure_data(2), measure_data(3), 0, 0, measure_data(4),
				...
				std::string detected_cube_2d_img_name =
					base_folder + "pred_3d_obj_overview/" + std::string(frame_index_c) + "_best_objects.jpg";
		...
		if (has_detected_cuboid)
		{
			...
			localcuboid->meas_quality = (1 - proposal_error + 0.5)
				/ 2;  // initial error 0-1, higher worse,  now change to [0.5,1] higher, better
...
int main(int argc, char* argv[])
{
	...
	if (online_detect_mode)
		cout << ("Online detect object mode !!\n");
	else
		cout << ("Offline read object mode !!\n");
